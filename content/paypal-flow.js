...
function findHostedCreateAccountButton() {
  const positivePatterns = [
    ...
    /sign\s*up|join\s+paypal/i,
    /pay\s+as\s+guest|checkout\s+as\s+guest|guest\s+checkout/i,
    /创建.*(?:账户|账号|帐号|邮箱)|新建.*(?:账户|账号|帐号|邮箱)|注册/i,
    /访客.*(?:付款|结账|继续)|以访客身份/i,
  ];
  const loginOnlyPattern = /log\s*in|login|sign\s*in|登录|登入/i;
  const controls = getVisibleControls('button, a, [role="button"], input[type="button"], input[type="submit"]');
  return controls.find((el) => {
    if (!isEnabledControl(el)) {
      return false;
    }
    const text = getActionText(el);
    const isCreateEntry = positivePatterns.some((pattern) => pattern.test(text));
    if (!isCreateEntry) {
      return false;
    }
    const isOnlyLoginEntry = loginOnlyPattern.test(text)
      && !/create|sign\s*up|join|guest|checkout|创建|新建|注册|访客/i.test(text);
    return !isOnlyLoginEntry;
  }) || null;
}
...
async function submitHostedPayLogin(payload = {}) {
  ...
  if (createAccountButton) {
    await delayOperation({ stepKey: 'plus-checkout-create', kind: 'click', label: 'hosted-paypal-create-account' }, async () => {
      simulateClick(createAccountButton);
    });
  ...
  const emailInput = await waitUntil(() => document.getElementById('email') || findEmailInput(), {
    intervalMs: 250,
    timeoutMs: createAccountClicked ? 10000 : 1000,
    timeoutMessage: 'PayPal hosted checkout 选择创建账号后未找到邮箱输入框。',
  }).catch(() => document.getElementById('email') || findEmailInput());
  if (!emailInput) {
