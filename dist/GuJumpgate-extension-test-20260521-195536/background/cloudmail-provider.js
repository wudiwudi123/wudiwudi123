(function cloudMailProviderModule(root, factory) {
  ...
  function createCloudMailProvider(deps = {}) {
    ...
    function buildCloudMailEduDomain(domain = '') {
      const normalizedDomain = normalizeCloudMailDomain(domain);
      if (!normalizedDomain) {
        return '';
      }
      return normalizedDomain.startsWith('edu.') ? normalizedDomain : `edu.${normalizedDomain}`;
    }

    function isCloudMailIllegalDomainError(error) {
      return /非法邮箱域名|invalid\s+(?:email\s+)?domain|domain/i.test(String(error?.message || error || ''));
    }
    ...
    async function fetchCloudMailAddress(state, options = {}) {
      ...
      const addressDomain = buildCloudMailEduDomain(ensuredConfig.domain);
      const address = `${requestedLocal}@${addressDomain}`.toLowerCase();
      const fallbackAddress = `${requestedLocal}@${ensuredConfig.domain}`.toLowerCase();
      ...
      try {
        ...
        if (/token|unauthor|401/i.test(String(err?.message || ''))) {
          ...
          await requestCloudMailJson(refreshed.config, '/api/public/addUser', { method: 'POST', payload }).catch(async (retryErr) => {
            if (!isCloudMailIllegalDomainError(retryErr) || fallbackAddress === address) {
              throw retryErr;
            }
            await requestCloudMailJson(refreshed.config, '/api/public/addUser', {
              method: 'POST',
              payload: { list: [{ email: fallbackAddress }] },
            });
            await addLog(`Cloud Mail：服务端暂不允许 ${addressDomain}，已在服务端创建 ${fallbackAddress}，本地继续使用 ${address}`, 'warn');
          });
        } else if (isCloudMailIllegalDomainError(err) && fallbackAddress !== address) {
          await requestCloudMailJson(ensuredConfig, '/api/public/addUser', {
            method: 'POST',
            payload: { list: [{ email: fallbackAddress }] },
          });
          await addLog(`Cloud Mail：服务端暂不允许 ${addressDomain}，已在服务端创建 ${fallbackAddress}，本地继续使用 ${address}`, 'warn');
