...
- `Cloud Mail` 既可以作为邮箱生成器直接创建 `local@edu.domain`，也可以作为收件 provider 轮询 `cloudMailReceiveMailbox`；当 `Mail = Cloud Mail` 且邮箱生成器不是 Cloud Mail 时，会优先轮询配置的接收邮箱，避免错误轮询注册邮箱本身。
...
2. 当 `邮箱生成 = Cloud Mail` 时，后台用管理员账号获取 Token，并通过 `/api/public/addUser` 创建随机本地名前缀邮箱；生成域名固定走 `edu.` 前缀，例如配置 `example.com` 时生成 `随机@edu.example.com`，配置本身已是 `edu.example.com` 时不会重复叠加。
